...
export default function UseCasesF({
  ...
  const scrollSlideIntoView = (i: number, smooth = true) => {
  ...
  const onTrackMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    if (!dragActive.current || !trackRef.current) return;
  ...
  const finishDrag = () => {
    const track = trackRef.current;
    if (!dragActive.current || !track) return;
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={`overflow-x-auto scrollbar-none scroll-smooth -mx-6 md:-mx-12 overscroll-x-contain touch-pan-x select-none ${isDragging ? "cursor-grabbing" : "cursor-grab"}`}
      >
        <div
          className="flex gap-4 md:gap-6 py-2 [--peek:7.5%] xl:[--peek:0px]"
          style={{ paddingInline: "max(var(--peek), calc((100% - 72rem) / 2))" }}
        >
          {useCases.map((uc, i) => {
            ...
            return (
              <div
                ...
                data-index={i}
                className="snap-center shrink-0 w-[85vw] xl:w-full max-w-6xl"
              >
                <div
