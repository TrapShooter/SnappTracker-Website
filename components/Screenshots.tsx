        ...
        {screenshots.length > 0 ? (
          <div className="flex gap-6 overflow-x-auto pb-4 justify-center">
            {screenshots.map((s) => (
              <div
                key={s.src}
                className="shrink-0 w-64 rounded-3xl overflow-hidden shadow-xl border border-zinc-100"
              >
...
function MockScreen({
  ...
  return (
    <div className="w-56 shrink-0 rounded-3xl overflow-hidden shadow-2xl border-4 border-zinc-800 bg-white">
      {/* Status bar */}
      <div className="bg-zinc-900 h-7 flex items-center justify-center">
        <div className="w-16 h-3 bg-zinc-700 rounded-full" />
      </div>
      {/* Header */}
      <div style={{ backgroundColor: color + "15" }} className="px-4 py-3 border-b border-zinc-100">
      ...
      {/* Tab bar */}
      <div className="border-t border-zinc-100 bg-white h-12 flex items-center justify-around px-4">
        {["⏱", "📋", "📊", "⚙️"].map((icon) => (
...
function MockTimerContent() {
  return (
    <div className="flex flex-col items-center gap-4 pt-4">
      <div className="text-3xl font-mono font-bold text-zinc-900">00:23:47</div>
      ...
      <div className="w-full rounded-xl bg-zinc-50 p-3 text-center">
        <p className="text-xs text-zinc-400">Cut</p>
        <p className="text-sm font-semibold text-zinc-700">✂ Mark session</p>
      </div>
    </div>
  );
}
...
function MockHistoryContent() {
  ...
  return (
    <div className="flex flex-col gap-2">
      {items.map((item, i) => (
        <div key={item} className="flex items-center gap-3 rounded-xl bg-zinc-50 p-2.5">
          <div className="h-2 w-2 rounded-full bg-[#6366f1]" />
          <div className="flex-1">
            <p className="text-xs font-medium text-zinc-800">{item}</p>
            <p className="text-[10px] text-zinc-400">{`${i + 1}h ${15 + i * 7}m`}</p>
          </div>
        </div>
      ))}
    </div>
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="text-center">
        <p className="text-2xl font-bold text-zinc-900">32h 14m</p>
